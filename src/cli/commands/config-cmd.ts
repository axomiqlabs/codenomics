  ...
  if (values.help || !sub) {
    console.log(
      [
        ...
        '',
        'Subcommands:',
        '  get <key>          read a merged config value by dotted key path',
        '  set <key> <value>  write a value to the user config (or project config with --project)',
        '  unset <key>        remove a key from the user config (or project config with --project)',
        '  list               print the full merged effective config as JSON',
        '  path               show user and project config file locations',
        '',
        'Options:',
        '  --project          target .codenomics.json in the current directory instead of the user config',
        '',
        'Examples:',
        '  get drivers.attentionUsdPerPrompt',
        '  set drivers.attentionUsdPerPrompt 8',
        '  set limits \'[{"id":"daily","metric":"costUsd","period":"day","max":50,"scope":"global"}]\'',
        '  unset pricing.gpt-5.5',
      ].join('\n'),
    );
